...
void set_config()
{
   GlobalConfig::instance()
     ...
     .setAfter(
       [](auto& f) { fmt::format_to(back_inserter(f), "你好我是end回调"); })
     .setFormatter(lblog::formatter::jsonFormatter);
}

void bench_start(const char*                  bench_name,
...
void test_multi_thread()
{
   set_config();
   std::thread th1{[]() {
      for (int i = 0; i < test_n; i++) warn("data:{},thread1", test_line);
   }};
   std::thread th2{[]() {
      for (int i = 0; i < test_n; i++) info("data:{},thread2", test_line);
   }};
   std::thread th3{[]() {
      for (int i = 0; i < test_n; i++) error("data:{},thread3", test_line);
   }};
   std::thread th4{[]() {
      for (int i = 0; i < test_n; i++) info("data:{},thread4", test_line);
   }};
   std::thread th5{[]() {
      for (int i = 0; i < test_n; i++) info("data:{},thread5", test_line);
   }};
   th1.join();
   th2.join();
   th3.join();
